...
export const LabTrackingManager: React.FC<LabTrackingManagerProps> = ({
  ...
  const handleSaveEntry = (e: React.FormEvent) => {
    ...
    if (editingEntryId) {
      ...
      onNotify?.('تم تعديل قراءة التحليل بنجاح ✏️');
    } else {
      // Add new
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-3xl bg-gradient-to-r from-teal-500/10 via-cyan-500/10 to-blue-500/10 dark:from-teal-950/30 dark:via-cyan-950/30 dark:to-blue-950/30 border border-teal-200/80 dark:border-teal-900/60 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-2xl bg-teal-600 hover:bg-teal-700 active:scale-95 text-white font-bold text-xs shadow-sm transition-all shrink-0"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <label className="flex items-center justify-between p-3.5 rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 cursor-pointer shadow-2xs hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
          <div className="space-y-0.5">
            <span className="text-xs font-bold text-slate-800 dark:text-slate-100 block">
              إظهار قسم التحاليل للعميل في تاب الجسم 👁️
            </span>
        ...
        <label className="flex items-center justify-between p-3.5 rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 cursor-pointer shadow-2xs hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
          <div className="space-y-0.5">
            <span className="text-xs font-bold text-slate-800 dark:text-slate-100 block">
              السماح للعميل بإضافة قراءات جديدة ✍️
            </span>
            <span className="text-[11px] text-slate-500 dark:text-slate-400">
      ...
      {isAdding && (
        <form
          ...
          className="p-5 rounded-3xl bg-white dark:bg-slate-900 border-2 border-teal-500/40 shadow-md space-y-4 animate-in fade-in duration-200"
        >
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              <span className="text-lg">🧪</span>
              <h4 className="font-bold text-slate-800 dark:text-slate-100 text-sm">
                {editingEntryId ? 'تعديل قراءة تحليل ✏️' : 'تسجيل قراءة تحليل جديدة ➕'}
              </h4>
            </div>
            <button
              type="button"
              ...
              <div className="p-3 rounded-2xl bg-teal-50/60 dark:bg-teal-950/30 border border-teal-200/60 dark:border-teal-900/40 text-xs text-teal-900 dark:text-teal-200 flex items-start gap-2">
                ...
                <div>
                  ...
                  {meta.rangeGuidance.hint && (
                    <span className="block text-[11px] text-teal-700 dark:text-teal-300 mt-0.5">
                      {meta.rangeGuidance.hint}
                    </span>
                  )}
                </div>
      ...
      {testKeys.length === 0 && !isAdding && (
        <div className="p-8 text-center rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 flex items-center justify-center text-2xl shadow-inner">
            🧪
          </div>
          <div>
          ...
          <div className="grid grid-cols-1 gap-3.5">
            {testKeys.map((key) => {
              ...
              return (
                <div
                  key={key}
                  className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs overflow-hidden transition-all"
                >
                  {/* Card Header Summary */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                            {meta.categoryLabel}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-200/60 dark:border-slate-800">
                      <div className="text-left">
                        ...
                        <div className="flex items-center justify-end gap-1.5 mt-0.5">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${evalResult.badgeClass}`}>
                            {evalResult.label}
                          </span>
                        </div>
                      </div>
