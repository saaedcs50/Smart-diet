  ...
  const steps: StepItem[] = [
    {
      ...
      title: `أهلاً بك يا ${plan.clientName || 'بطل'} في رفيقك الغذائي!`,
      subtitle: 'نظام المتابعة الذكي بإشراف د. شيماء',
      description: 'هذا التطبيق صُمم خصيصاً لمساعدتك على الالتزام بخطتك الغذائية اليومية بسهولة تامة، بدون حرمان أو تعقيد في الحسابات، مع إمكانية استبدال الأكلات بالبدائل المتاحة في بيتك ومتابعة تقدمك لحظة بلحظة.',
    ...
    },
    {
      ...
      title: 'متابعة يومك خطوة بخطوة',
      subtitle: 'سجل إنجازاتك اليومية وشاهد تقدمك اللحظي',
      description: 'شاشة "اليوم" هي صفحتك الرئيسية لمتابعة إنجاز الوجبات والعادات اليومية.',
      ...
      proTip: 'يوم الراحة (Free Day ❄️): يمكنك تفعيله في الأيام المفتوحة للحفاظ على استمرارية سلسلتك دون التأثير على إحصائياتك!'
    },
    {
      id: 'body_tab',
    ...
    },
    {
      ...
      badgeColor: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800',
      title: 'متابعة التطور ومشاركة الدكتورة',
      ...
      details: [
        ...
        },
        {
          ...
          description: 'في يوم المتابعة، اضغط على زر "إرسال التقرير للدكتورة عبر واتساب" ليقوم التطبيق بتجهيز رسالة منسقة وجاهزة لإرسالها مباشرة.',
          icon: <Share2 className="w-5 h-5 text-emerald-500" />,
          highlight: 'متابعة مباشرة مع العيادة'
        }
      ],
      proTip: 'إرسال التقرير الأسبوعي للدكتورة يساعد في تحديث خطتك وتعديل السعرات لكسر أي ثبات وزن بشكل مستمر!'
    },
    {
      id: 'pwa_and_help',
      ...
      details: [
        {
          ...
          description: 'اضغط على زر المشاركة أو القائمة في المتصفح ⬅️ ثم اختر "إضافة إلى الشاشة الرئيسية" (Add to Home Screen).',
          icon: <Smartphone className="w-5 h-5 text-emerald-500" />
        },
        {
          title: 'زر المساعدة بجانب كل ميزة (❓)',
          description: 'ستجد علامة استفهام بجانب كل عنوان في التطبيق — اضغط عليها في أي وقت لمعرفة كل تفاصيل استخدام الميزة وتوجيهات الدكتورة.',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/80 backdrop-blur-md overflow-y-auto" dir="rtl">
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-auto flex flex-col max-h-[92vh]"
      >
        {/* Header Bar */}
        ...
        <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 flex">
          {steps.map((step, idx) => (
            <button
              ...
              className={`h-full transition-all duration-300 flex-1 cursor-pointer ${
                idx === currentStep
                  ? 'bg-emerald-500'
                  : idx < currentStep
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 flex-1">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Title & Subtitle Card */}
              <div className="flex items-start gap-4 p-4 rounded-2xl bg-gradient-to-br from-emerald-50/70 via-teal-50/40 to-white dark:from-emerald-950/30 dark:via-slate-900 dark:to-slate-900 border border-emerald-100/80 dark:border-emerald-900/50">
                <div className="p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-emerald-100/50 dark:border-slate-700 shrink-0">
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {currentStepData.details.map((detail, dIdx) => (
                    ...
                    >
                      <div className="p-2 bg-white dark:bg-slate-800 rounded-lg shadow-2xs border border-slate-200/60 dark:border-slate-700 shrink-0 mt-0.5">
                        {detail.icon}
                      </div>
        ...
        <div className="p-4 sm:p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/80 flex items-center justify-between gap-3">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold border transition-all cursor-pointer ${
              currentStep === 0
                ...
                : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 shadow-2xs'
            }`}
          >
          ...
          <button
            onClick={handleNext}
            className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold text-white shadow-md transition-all cursor-pointer ${
              isLastStep
                ? 'bg-emerald-600 hover:bg-emerald-700 active:scale-95 shadow-emerald-500/20'
                : 'bg-slate-900 dark:bg-emerald-600 hover:bg-slate-800 dark:hover:bg-emerald-700 active:scale-95'
            }`}
          >
            {isLastStep ? (
              <>
