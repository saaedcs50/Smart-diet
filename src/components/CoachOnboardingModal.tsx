  ...
  const steps: CoachStepItem[] = [
    {
      ...
      subtitle: 'نظام إدارة الخطط والبدائل والمتابعة بإشراف د. شيماء',
      ...
      clinicalTip: 'يمكنكِ دائماً إعادة فتح هذا الدليل في أي وقت من زر "دليل لوحة الطبيبة 📖" الموجود في أعلى اللوحة.'
    },
    {
      id: 'step1_presets',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto" dir="rtl">
      <motion.div
        ...
        className="relative w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-auto flex flex-col max-h-[92vh]"
      >
        {/* Top Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-emerald-50/70 via-teal-50/40 to-white dark:from-emerald-950/40 dark:via-slate-900 dark:to-slate-900 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0 border border-emerald-500/20 shadow-2xs">
              <Stethoscope className="w-5 h-5" />
            </div>
            <div>
              <span className="text-[10px] font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-300">
                دليل الطبيبة والأخصائية (د. شيماء)
              </span>
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-2xl text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center transition-colors shrink-0 cursor-pointer active:scale-95"
            title="إغلاق الدليل"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="space-y-1.5">
                <span className={`inline-block text-[10px] font-black px-2.5 py-0.5 rounded-full border ${current.badgeColor}`}>
                  {current.badge}
                </span>
              ...
              <div className="space-y-2.5">
                {current.details.map((detail, idx) => (
                  <div
                    key={idx}
                    className="p-3 sm:p-3.5 rounded-2xl bg-white dark:bg-slate-850 border border-slate-100 dark:border-slate-750 shadow-2xs space-y-1"
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {detail.highlight && (
                        <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 shrink-0">
                          {detail.highlight}
                        </span>
                      )}
                    </div>
              ...
              {current.clinicalTip && (
                <div className="p-3.5 rounded-2xl bg-emerald-500/10 dark:bg-emerald-500/15 border border-emerald-300/60 dark:border-emerald-700/50 flex items-start gap-2.5 text-xs text-emerald-900 dark:text-emerald-200">
                  ...
                  <div>
                    <strong className="font-black">توجيه د. شيماء الإكلينيكي: </strong>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/90 flex items-center justify-between gap-3 shrink-0">
          <button
            ...
            className={`min-h-[42px] px-3.5 rounded-xl font-black text-xs transition-all flex items-center gap-1.5 cursor-pointer ${
              isFirstStep
                ...
                : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 active:scale-95'
            }`}
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleNext}
              className="min-h-[42px] px-5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs sm:text-sm shadow-md transition-all flex items-center gap-2 cursor-pointer active:scale-95"
            >
