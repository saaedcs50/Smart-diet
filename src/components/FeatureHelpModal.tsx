...
import { HelpCircle, X, Sparkles, Lightbulb, CheckCircle2, ChevronRight } from 'lucide-react';
...
export const FEATURE_GUIDES: Record<string, FeatureHelpInfo> = {
  scoreCard: {
    ...
    summary: 'يحسب لك التطبيق نسبة التزامك اليومية المئوية بناءً على إنجازك للوجبات، كمية الماء، التمارين، والمكملات المقررة من الدكتورة شيماء.',
    howToUse: [
      ...
      'زر يوم الراحة (Free Day ❄️): يمكنك استخدامه في أيام الإجازة المقررة للحفاظ على سلسلتك دون التأثير على تقييمك العام.'
    ],
  ...
  mealsList: {
    ...
    howToUse: [
      ...
      'تسجيل مستوى الشبع والجوع: يمكنك تقييم شعورك قبل وبعد الوجبة لمساعدة الدكتورة في ضبط الكميات لاحقاً.'
    ],
  ...
  symptomsAndNotes: {
    id: 'symptomsAndNotes',
    title: 'ملاحظات الأعراض والرسالة المباشرة للدكتورة',
    ...
    howToUse: [
      ...
      'اكتب في المربع النصي أي سؤال أو ملاحظة تود أن تقرأها الدكتورة شيماء في تقريرك.'
    ]
  },
  bodyMeasurements: {
    id: 'bodyMeasurements',
  ...
  photoCompare: {
    ...
    howToUse: [
      ...
      'اختر صورتين للمقارنة ومشاهدة الفارق المذهل في شكل الجسم.'
    ],
    doctorNote: 'جميع الصور تحفظ محلياً على هاتفك فقط لحفظ خصوصيتك التامة.'
  },
  whatsappReport: {
    id: 'whatsappReport',
    title: 'إرسال تقرير المتابعة للدكتورة عبر واتساب',
    badge: 'المتابعة الأسبوعية',
    summary: 'توليد تقرير احترافي منسق يلخص التزامك، وزنك، شرب الماء، والرياضة وإرساله بنقرة واحدة.',
    howToUse: [
      'في يوم المتابعة، اضغط على زر "إرسال التقرير للدكتورة عبر واتساب".',
      'سيفتح التطبيق محادثة الواتساب بنص جاهز ومنسق يتضمن كل إحصائياتك لمراجعتها معك.'
    ],
    proTip: 'إرسال التقرير بانتظام يساعد الدكتورة في تعديل خطتك وتفادي أي ثبات وزن مبكراً.'
  },
  importPlan: {
    id: 'importPlan',
    title: 'إدخال وتحديث الخطة الغذائية',
    badge: 'تحديث الخطة',
    summary: 'استيراد خطتك الغذائية الجديدة المرسلة من عيادة د. شيماء بضغطة زر أو عبر رمز الاستجابة السريعة.',
    howToUse: [
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/80 backdrop-blur-sm overflow-y-auto" dir="rtl">
      <motion.div
        ...
        className="relative w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-auto flex flex-col max-h-[90vh]"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-emerald-50/60 via-teal-50/30 to-white dark:from-emerald-950/30 dark:via-slate-900 dark:to-slate-900">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              {guide.badge && (
                <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                  {guide.badge}
                </span>
              )}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-4 text-slate-700 dark:text-slate-200 text-xs sm:text-sm leading-relaxed flex-1">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {guide.howToUse.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-2.5 p-3 rounded-xl bg-white dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 shadow-2xs"
                >
                  <div className="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-black text-[11px] shrink-0 mt-0.5">
                    {idx + 1}
                  </div>
          ...
          {guide.doctorNote && (
            <div className="p-3.5 rounded-2xl bg-emerald-500/10 dark:bg-emerald-500/15 border border-emerald-300/60 dark:border-emerald-700/50 flex items-start gap-2.5 text-xs text-emerald-900 dark:text-emerald-200">
              ...
              <div>
                <strong className="font-bold">توجيه د. شيماء: </strong>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/80 flex items-center justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm shadow-md transition-all active:scale-95"
          >
            فهمت، شكراً
          </button>
        </div>
