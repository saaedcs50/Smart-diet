...
export const MEDICATION_SLOT_OPTIONS: SlotOptionMeta[] = [
  ...
  { slot: 'with_dinner', label: 'مع العشاء', category: 'dinner', icon: '🍽️' },
  { slot: 'after_dinner', label: 'بعد العشاء', category: 'dinner', icon: '🌙' },

  // عام / بدون ربط وجبة
  { slot: 'morning', label: 'صباحاً (بدون ربط وجبة)', category: 'general', icon: '☀️' },
  { slot: 'evening', label: 'مساءً (بدون ربط وجبة)', category: 'general', icon: '🌆' },
  { slot: 'before_sleep', label: 'قبل النوم', category: 'general', icon: '🛏️' },
...
export const WEIGHT_EFFECT_LABELS: Record<WeightEffect, { label: string; icon: string; color: string }> = {
  loss: { label: 'قد يساهم في نزول الوزن', icon: '⚖️⬇️', color: 'text-emerald-600 dark:text-emerald-400' },
  gain: { label: 'قد يسبب زيادة وزن أو احتباس سوائل', icon: '⚖️⬆️', color: 'text-rose-600 dark:text-rose-400' },
  none: { label: 'محايد على الوزن', icon: '⚖️➖', color: 'text-slate-500' },
  unknown: { label: 'غير محدد', icon: '❓', color: 'text-slate-400' },
};
